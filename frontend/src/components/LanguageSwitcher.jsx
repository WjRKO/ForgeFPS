    ...
    <div className="flex items-center gap-1" data-testid="language-switcher">
      ...
      {LANGS.map((l) => (
        <button key={l.code} data-testid={`lang-${l.code}`} onClick={() => i18n.changeLanguage(l.code)}
          ...
          className={`flex items-center gap-1 text-xs font-bold px-2.5 py-3 rounded-sm transition-all duration-200 hover:-translate-y-0.5 ${current === l.code ? "bg-[#E5FF00] text-black shadow-[0_2px_10px_rgba(229,255,0,0.35)]" : "text-zinc-400 hover:text-white"}`}>
