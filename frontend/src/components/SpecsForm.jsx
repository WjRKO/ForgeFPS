...
export default function SpecsForm({ initial = {}, onSaved, onCancel }) {
  ...
  const detect = () => {
    ...
    toast.success("Rilevamento dal browser completato. Controlla e correggi se serve.");
  };

  const save = async () => {
    if (!f.cpu && !f.gpu) { toast.error("Inserisci almeno CPU o GPU."); return; }
    setSaving(true);
    try {
      const { data } = await api.post("/pc-specs", { data: f, source: "manual" });
      toast.success("Specifiche salvate!");
      onSaved?.(data);
    } catch {
  ...
  return (
    <div className="bg-[#0F0F12] border border-[#2A2A35] p-6" data-testid="specs-form">
      <div className="flex items-center justify-between mb-4">
        <div className="text-xs uppercase tracking-[0.2em] text-zinc-500">Specifiche del PC</div>
        <div className="flex gap-2">
          <button data-testid="detect-browser-btn" onClick={detect}
            ...
            <ScanLine size={14} className="text-[#E5FF00]" /> Rileva dal browser
          </button>
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        ...
        <div><label className={LABEL}>RAM</label>
          <select data-testid="specs-ram" value={f.ram} onChange={set("ram")} className={FIELD}>
            {RAM_OPTIONS.map((o) => <option key={o} value={o}>{o || "— seleziona —"}</option>)}
          </select></div>
        <div><label className={LABEL}>Risoluzione</label>
          <select data-testid="specs-res" value={f.resolution} onChange={set("resolution")} className={FIELD}>
            {RES_OPTIONS.map((o) => <option key={o} value={o}>{o || "— seleziona —"}</option>)}
          </select></div>
        <div className="sm:col-span-2"><label className={LABEL}>Sistema operativo</label>
      ...
      <button onClick={() => setAdv((a) => !a)} className="flex items-center gap-1 text-xs text-zinc-500 hover:text-[#E5FF00] mt-4">
        <ChevronDown size={14} className={`transition-transform ${adv ? "rotate-180" : ""}`} /> Avanzate (scheda madre, socket, chipset)
      </button>
      {adv && (
        <div className="grid sm:grid-cols-3 gap-4 mt-3">
          <div><label className={LABEL}>Scheda madre</label>
          ...
          <div><label className={LABEL}>Socket</label>
            <input data-testid="specs-socket" value={f.cpu_socket} onChange={set("cpu_socket")} placeholder="es. AM4" className={FIELD} /></div>
          <div><label className={LABEL}>Chipset</label>
            <input data-testid="specs-chipset" value={f.chipset} onChange={set("chipset")} placeholder="es. X570" className={FIELD} /></div>
        </div>
      )}

      <div className="flex items-center justify-between mt-5">
        <p className="text-xs text-zinc-600 max-w-sm">Per Health Score, temperature e ottimizzazioni reali usa il Desktop Agent.</p>
        <button data-testid="save-specs-btn" onClick={save} disabled={saving}
          className="flex items-center gap-2 bg-[#E5FF00] text-black font-bold px-5 py-2.5 hover:bg-[#D4EC00] transition-colors disabled:opacity-60">
          {saving ? <Loader2 size={16} className="animate-spin" /> : <Save size={16} />} Salva specifiche
        </button>
      </div>
    </div>
  );
