...
function timeAgo(iso) {
  ...
  try {
    ...
    return `${Math.floor(s / 86400)}${isEn() ? "d" : "g"} ${suf}`;
  } catch { return "—"; }
}
...
export default function PasswordResetsPanel() {
  ...
  const load = async () => {
    ...
    try {
      ...
      toast.error((isEn() ? "Unable to load reset tokens: " : "Impossibile caricare i reset token: ") + (e?.response?.data?.detail || e.message));
      setItems([]);
  ...
  const copy = async (relLink, idx) => {
    ...
    try {
      ...
      setCopiedIdx(idx);
      toast.success(isEn() ? "Link copied to clipboard" : "Link copiato negli appunti");
      setTimeout(() => setCopiedIdx(-1), 2000);
    } catch {
      toast.error(isEn() ? "Copy failed — select and copy manually" : "Copia fallita — seleziona e copia manualmente");
    }
  };
  ...
  return (
    <div className="bg-[#0F0F12] border border-[#2A2A35] p-6 mt-8" data-testid="password-resets-panel">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1 max-w-2xl">
            {isEn() ? "Last 20 links generated by users. Real email delivery is TODO — meanwhile copy the link and send it to the recipient yourself (Discord DM / personal email)." : "Ultimi 20 link generati dagli utenti. L'invio email vero e' TODO — nel frattempo copia il link e mandalo tu al destinatario (Discord DM / email personale)."}
          </p>
        </div>
      ...
      {loading && items === null ? (
        <div className="text-center py-8 text-zinc-500 text-sm">{isEn() ? "Loading..." : "Caricamento..."}</div>
      ) : !items || items.length === 0 ? (
        <div className="text-center py-8 text-zinc-500 text-sm">
          {isEn() ? "No resets requested in the last 30 days." : "Nessun reset richiesto negli ultimi 30 giorni."}
        </div>
      ) : (
          ...
          <table className="w-full text-sm" data-testid="password-resets-table">
            <thead>
              <tr className="text-xs uppercase tracking-widest text-zinc-500 border-b border-[#2A2A35]">
                ...
                <th className="text-left py-2 px-3">{isEn() ? "Status" : "Stato"}</th>
                <th className="text-right py-2 px-3">{isEn() ? "Action" : "Azione"}</th>
              </tr>
            </thead>
            <tbody>
              {items.map((it, i) => {
