...
export default function Upgrade() {
  ...
  const [goal, setGoal] = useState("gaming 1440p ad alti FPS");
  ...
  const trackParts = async () => {
    ...
    try {
      ...
      toast.success(`${data.tracked} componenti aggiunti al Price Tracker`);
    } catch { toast.error("Errore nel tracking"); } finally { setTracking(false); }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto fade-up">
      <div className="mb-6"><div className="text-xs uppercase tracking-[0.2em] text-zinc-500 mb-2">// Upgrade & FPS</div>
        <h1 className="font-display font-black text-3xl tracking-tighter">Potenzia il tuo setup</h1></div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="bg-[#0F0F12] border border-[#2A2A35] p-6">
          <div className="text-xs uppercase tracking-[0.2em] text-zinc-500 mb-4 flex items-center gap-2"><Rocket size={14} className="text-[#E5FF00]" /> Consiglio di upgrade</div>
          <div className="mb-4">
            <div className="flex justify-between text-xs uppercase tracking-widest text-zinc-500 mb-2"><span>Budget upgrade</span><span className="text-[#E5FF00] font-bold">€{budget}</span></div>
          ...
          <label className="text-xs uppercase tracking-widest text-zinc-500">Obiettivo</label>
          ...
          <button data-testid="analyze-upgrade-btn" onClick={analyze} disabled={loading || !hasSpecs}
            ...
            {loading ? <Loader2 size={16} className="animate-spin" /> : <TrendingUp size={16} />} Analizza upgrade
          </button>
          {!hasSpecs && <p className="text-xs text-zinc-500 mt-2">Richiede le specifiche del PC.</p>}
          ...
          {upg && (
            <div className="mt-5 fade-up">
              <div className="bg-black border border-[#2A2A35] p-4 mb-3">
                <div className="text-xs uppercase tracking-widest text-zinc-500">Collo di bottiglia</div>
              ...
              {upg.recommendations.map((r, i) => (
                <div key={i} className="border border-[#1A1A24] p-3 mb-2" data-testid={`upg-rec-${i}`}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-xs uppercase tracking-widest text-[#E5FF00]">{r.category}</span>
                    <span className={`text-xs font-bold uppercase px-2 py-0.5 ${PRIO[r.priority] || "bg-zinc-700/30 text-zinc-400"}`}>{r.priority}</span>
                  </div>
              ...
              {upg.keep?.length > 0 && (
                <div className="text-xs text-zinc-500 mt-2 flex items-start gap-2"><CheckCircle2 size={13} className="text-[#00FF66] mt-0.5" /> Va bene: {upg.keep.join(", ")}</div>
              )}
              <div className="flex items-center justify-between mt-4">
                <div className="font-display font-black text-lg">Totale: <span className="text-[#E5FF00]">€{upg.estimated_total}</span></div>
                <button data-testid="track-upgrade-btn" onClick={trackParts} disabled={tracking}
                  ...
                  {tracking ? <Loader2 size={14} className="animate-spin" /> : <LineIcon size={14} />} Traccia i pezzi
                </button>
              </div>
            </div>
          )}
        ...
        <div className="bg-[#0F0F12] border border-[#2A2A35] p-6 h-fit">
          <div className="text-xs uppercase tracking-[0.2em] text-zinc-500 mb-4 flex items-center gap-2"><Gauge size={14} className="text-[#E5FF00]" /> Stima FPS per gioco</div>
          <label className="text-xs uppercase tracking-widest text-zinc-500">Gioco</label>
          <input data-testid="fps-game-input" value={game} onChange={(e) => setGame(e.target.value)} placeholder="es. Fortnite, Cyberpunk 2077..."
          ...
          <button data-testid="estimate-fps-btn" onClick={estimate} disabled={fpsLoading}
            className="w-full flex items-center justify-center gap-2 bg-[#E5FF00] text-black font-bold py-3 hover:bg-[#D4EC00] transition-colors disabled:opacity-60">
            {fpsLoading ? <Loader2 size={16} className="animate-spin" /> : <Gauge size={16} />} Stima FPS
          </button>
          ...
          {fps && (
            <div className="mt-5 fade-up">
              ...
              <div className="mt-4 text-sm"><span className="text-zinc-500">Consigliato: </span><span className="text-[#E5FF00] font-bold">{fps.recommended_preset}</span></div>
