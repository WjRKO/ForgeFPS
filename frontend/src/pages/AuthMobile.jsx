...
export default function AuthMobile() {
  ...
  useEffect(() => {
    ...
    if (!token) { setState("error"); setErrorMsg(isEn() ? "Invalid link: missing token" : "Link non valido: token mancante"); return; }
    (async () => {
      try {
        ...
        setState("error");
        setErrorMsg(e?.response?.data?.detail || (isEn() ? "Link expired or already used" : "Link scaduto o già usato"));
      }
    })();
  }, [params, navigate]);
        ...
        {state === "error" && (
          <>
            ...
            <p className="text-zinc-500 text-xs">
              {isEn() ? "Magic links last 5 minutes and are single-use. Go back to your PC and regenerate the QR code." : "I magic link durano 5 minuti e sono a uso singolo. Torna sul PC e rigenera il QR code."}
            </p>
            <button
